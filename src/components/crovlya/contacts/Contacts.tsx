...
export const Contacts: React.FC = () => {
  ...
  const sendToTelegram = async (data: FormData) => {
    ...
    const chatId = '-4841505337';

    console.log('Отправка в Telegram:', { botToken, chatId, data });

    const message = `📞 Перезвоните
...
⏰ Время заявки: ${new Date().toLocaleString('ru-RU')}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Ошибка отправки в Telegram: ${response.status} - ${errorData}`);
      }
          ...
          <div className="w-full max-w-[400px]">
            <div className="flex items-start gap-3">
              <input
                type="checkbox"
                id="privacyConsent"
                className="mt-1 w-12 h-12 sm:w-4 sm:h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
